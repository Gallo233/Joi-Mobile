...
## DEC-040 — A turn is bounded by silence, not by duration

- **Status:** Accepted
- **Date:** 2026-08-18
- **Decision:** `ChatSessionController` watches each turn for silence. Twenty seconds with no event cancels the turn and reports `ChatSessionError.timedOut`, a retryable state distinct from an unavailable service. Every event resets the watch, so a slow but live stream is never cut off. A user stop is never reported as a timeout.
- **Why silence rather than a total deadline.** A long answer on a good connection and a dead connection look identical to a wall clock. Bounding silence separates them: a turn that is still producing is still working, however long it takes, and a turn producing nothing is over whatever the total elapsed time says.
- **Why progress is counted, not timestamped.** The watch asks "did anything happen while I slept". A counter needs no clock, cannot drift, and has no timezone or monotonicity questions attached to it.
- **Why a stop and a stall must be told apart.** Both surface as cancellation. Reporting a stop as a timeout would tell the user the network failed when they were the one who stopped it — so only the watchdog, which knows it fired, may label the error.
- **What the degraded copy says.** A slow connection is not an outage, and the message says so, offers a retry, and names the cached walk as what still works with no network. That last part is true of this product specifically: the walk is the only offline content it has.
- **Two testing lessons, recorded because both cost real time.** A generic `Result<T, Error>?` task group sent Swift's type checker into an inference long enough to read as a hung test run rather than the slow compile it was. And a stalled turn lives inside an *unstructured* task, so cancelling the caller cannot reach it and `withTaskGroup` — which always awaits its children — hangs on exactly the failure the test exists to report. Both suites now poll a box the turn writes into and never await the stuck task, so deleting the watchdog fails in seconds with a named message.
- **`FAIL-024` stays partial.** Nothing observes reachability, so the app cannot enter a degraded mode before a turn stalls, and Chat has no cached answer to fall back to. Adding that needs a network monitor and a decision about what a cached conversational answer would even be — neither of which is a timeout.

## DEC-041 — A correction outlives the readings that disagree with it

- **Status:** Accepted
- **Date:** 2026-08-18
- **Decision:** `PlaceResolver` proposes which of a cached tour's stops the walker is at, from position, accuracy and the stop list. A confident proposal settles itself; an ambiguous one keeps its whole candidate set and waits. A user's correction applies locally and immediately and is **not** retired by later readings, while an automatic confirmation is retired by walking away.
- **The place database was already there.** A travel pack declares what its places are and where they sit, so arrive-and-tell over a downloaded route needs no gazetteer and no network. The slices that built packs and stops were not aiming at this, and it fell out of them.
