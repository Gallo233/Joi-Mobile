...
- **Confirmed on both sides (2026-08-14):** this stopped being an inference the moment the Kotlin core compiled. Its `content identity of a materialised tree` case passes on every vector *including* `decomposable-path`, because it normalizes to NFC before hashing; the Swift twin has to skip that one case to stay green. Two implementations sharing no source, one corpus file, disagreeing precisely where the rule predicted — and the platform that does not conform is iOS.
- **Open condition:** the rule as written — hash NFC bytes, so identity is a property of the package and not of the volume — is the one the Android core implements, and the `decomposable-path` vector is carried as **non-normative** until iOS matches it. Changing the iOS side re-identifies any installed package with a decomposable filename, which is a migration question and not a bug fix, so it needs its own decision. Until then a character whose files carry accented Latin or kana with dakuten has two identities. Every ASCII package — which is every fixture in this repository today — is unaffected.
